...
#include <iostream>                                // start of programm
	...
	int main()
	{
		...
		for (i = 0; i < 3; i++)
		{
			...
			{   
				std::cout << "matrix [" <<i<<","<<j<<"]";
				std::cin >> array1[i][j];                            // initiliazing array by user..
				std::cout << "\n";
			}
		
		}
	
		for (i = 0; i < 3; i++)                                  // comparing matrix element that identity or not
		{
			for (j = 0; j < 3; j++)
			{
				if (i == j && array1[i][j] != 1)
